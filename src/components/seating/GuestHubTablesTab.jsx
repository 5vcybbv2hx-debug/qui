import { List, Grid2x2, Plus, Sparkles, MessageCircle } from 'lucide-react';
                            ...
                            <button key={table.id} onClick={() => onTableSelect(table)}
                                ...
                                <div className="flex items-start justify-between gap-1">
                                    ...
                                    <div className="flex flex-col items-end gap-1">
                                        <span className={cn('w-2.5 h-2.5 rounded-full shrink-0', effDot)} />
                                        {noteExists && (
                                            <span title={getNotesText(table.id)}>
                                ...
                                <div>
                                    <p className="text-xs font-medium">{effLabel}</p>
                    ...
                    {tableWithStatus.map(({ table, status, reservation }) => {
                        ...
                        return (
                            <button key={table.id} onClick={() => onTableSelect(table)}
                                ...
                                <div className={cn('w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg shrink-0 relative', effColor)}>
                                    {table.number}
                                    {noteExists && (
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {noteExists && (
                                        <p className="text-xs text-muted-foreground italic truncate mt-0.5" title={getNotesText(table.id)}>
                                            💬 {getNotesText(table.id)}
                                        </p>
                                    )}
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                    <span className={cn('text-xs px-2 py-1 rounded-full border', effColor)}>{effLabel}</span>
                                    {permissions.isManager && (
