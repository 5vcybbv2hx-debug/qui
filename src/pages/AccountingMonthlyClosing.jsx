...
export default function AccountingMonthlyClosing() {
    ...
    const { data: cashbookEntries = [] } = useQuery({ queryKey: ['cashbook-entries'], queryFn: () => base44.entities.CashbookEntry.list('-date', 500), staleTime: STALE.MEDIUM });
    const { data: receipts = [] } = useQuery({ queryKey: ['accounting-receipts'], queryFn: () => base44.entities.AccountingReceipt.list('-receipt_date', 500), staleTime: STALE.MEDIUM });
    const { data: creditorInvoices = [] } = useQuery({ queryKey: ['creditor-invoices'], queryFn: () => base44.entities.CreditorInvoice.list('-invoice_date', 300), staleTime: STALE.MEDIUM });
    const { data: dailyRevenues = [] } = useQuery({ queryKey: ['daily-revenues'], queryFn: () => base44.entities.DailyRevenue.list('-date', 365), staleTime: STALE.MEDIUM });
    const { data: closings = [] } = useQuery({ queryKey: ['monthly-closings'], queryFn: () => base44.entities.MonthlyClosing.list('-year'), staleTime: STALE.MEDIUM });
    const { data: recurringExpenses = [] } = useQuery({ queryKey: ['recurring-expenses'], queryFn: () => base44.entities.RecurringExpense.list('title'), staleTime: STALE.MEDIUM });
    const { data: recurringOccurrences = [] } = useQuery({ queryKey: ['recurring-expense-occurrences'], queryFn: () => base44.entities.RecurringExpenseOccurrence.list('-month'), staleTime: STALE.MEDIUM });
    const { data: liabilities = [] } = useQuery({ queryKey: ['liabilities'], queryFn: () => base44.entities.Liability.list('-due_date'), staleTime: STALE.MEDIUM });
