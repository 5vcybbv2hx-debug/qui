...
import { fillLegalTemplate, PRIVACY_POLICY_TEMPLATE } from '@/lib/legalContent';
...
export default function LegalPrivacy() {
  ...
  const filledText = fillLegalTemplate(PRIVACY_POLICY_TEMPLATE, companyInfo);
  ...
  return (
    <div className="min-h-screen bg-background pb-6">
      ...
      <div className="sticky top-0 z-40 bg-card border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-foreground">Datenschutzerklärung</h1>
      </div>
            ...
            <CardContent className="pt-6">
              <div className="prose prose-sm dark:prose-invert max-w-none">
                {filledText.split('\n').map((line, idx) => {
                  ...
                  if (line.startsWith('-')) {
                    return (
                      <li key={idx} className="text-foreground text-sm ml-4 py-0.5">
                        {line}
                      </li>
                    );
                  }
